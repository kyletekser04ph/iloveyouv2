module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["100064254475244","61554661960172"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only isami can do it.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
