...
module.exports = {
	...
	onStart: async function ({ message, getLang, threadsData, usersData, dashBoardData, globalData }) {
		...
		const pathThreads = `database/data/threadsData.json`;
		const pathUsers = `database/data/usersData.json`;
		const pathDashBoard = `database/data/dashBoardData.json`;
		const pathGlobal = `database/data/globalData.json`;
